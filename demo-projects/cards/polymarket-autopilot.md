...
```text
Start the Polymarket Autopilot: Automated Paper Trading demo project from the malaclaw starter `polymarket-autopilot`. Manually monitoring prediction markets for arbitrage opportunities and executing trades is time-consuming and requires constant attention. You want to test and refine trading strategies without risking real capital. Use `autonomous-startup` as the initial entry team. Review STARTER.md, confirm any missing external integrations, then break the work into clear team tasks.
```

## Suggested Flow

1. Inspect the starter with `malaclaw starter show polymarket-autopilot`.
2. Initialize it with `malaclaw starter init polymarket-autopilot <dir>`.
