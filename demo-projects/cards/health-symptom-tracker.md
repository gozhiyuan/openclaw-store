...
```text
Start the Health & Symptom Tracker demo project from the malaclaw starter `health-symptom-tracker`. Identifying food sensitivities requires consistent logging over time, which is tedious to maintain. You need reminders to log and analysis to spot patterns. Use `autonomous-startup` as the initial entry team. Review STARTER.md, confirm any missing external integrations, then break the work into clear team tasks.
```

## Suggested Flow

1. Inspect the starter with `malaclaw starter show health-symptom-tracker`.
2. Initialize it with `malaclaw starter init health-symptom-tracker <dir>`.
